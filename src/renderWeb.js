...
function renderContactSection(cvData) {
  ...
  document.getElementById("contact").innerHTML = `
    📍 <span data-testid="text-location">${cvData.location}</span> <br/> 
    ✉️ <span class="text-black-500 cursor-pointer hover:underline hover:text-blue-500 transition-colors duration-150" title="Copy email" data-testid="text-email" onclick="navigator.clipboard.writeText('${cvData.email}').then(() => alert('Email copied!'))">${cvData.email}</span> · 
    📞 <a href="https://wa.me/${whatsappPhone}" class="text-green-500 hover:underline hover:text-green-600 transition-colors duration-150" target="_blank" rel="noopener noreferrer" data-testid="link-whatsapp">Chat on WhatsApp</a><br/>
    <a href="${cvData.github}" class="text-blue-500 inline-flex items-center gap-1 hover:underline hover:text-blue-600 transition-colors duration-150" target="_blank" rel="noopener noreferrer" data-testid="link-github">
      <i class="fab fa-github"></i> GitHub
    </a> · 
    <a href="${cvData.linkedin}" class="text-blue-500 inline-flex items-center gap-1 hover:underline hover:text-blue-600 transition-colors duration-150" target="_blank" rel="noopener noreferrer" data-testid="link-linkedin">
